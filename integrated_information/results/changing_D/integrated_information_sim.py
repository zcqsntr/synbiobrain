...
D = 3e-6 #cm^2/s
for D in [D/100, D/10, D, D*10, D*100]:
...
#plot() # comment this to stop plotting
    ...
    np.save('IIs_' + str(D) + '.npy', np.array(IIs))
    np.save( 'MIs_' + str(D) + '.npy', np.array(MIs))
    np.save( 'IIs_modified_' + str(D) + '.npy', np.array(II_modified))
    plt.figure()
    ...
    plt.savefig('Int_inf_' + str(D) + '.png')
    #plt.show()
