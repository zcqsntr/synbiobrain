...
grid = SynBioBrainCUDA(grid_corners, nx, ny, checkerboard = True, dtype = 'float32')
...
input_indeces = np.array([0,2,4,6,8])
...
#ps = [10**(-1.9),10**(-0.6) ,10**(-2), 10**(-0.4)] THE REAL ONES BASED ON BAND PASS
ps = [10**(-1.9),10**(-1.5) ,10**(-1.9), 10**(-1.5)]
...
with Timer() as t:
    ...
    '''
    overall_Us, overall_Us1, overall_activated = grid.synbio_brain(heated_element, grid_corners, node_dim, input_indeces, output_indeces, params)
...
if plot_u2:
    ...
    ax.set_xlabel('X position (cm)')
    ax.set_ylabel('[AHL]')
    plot = ax.plot_trisurf(grid.vertex_positions[:,0], grid.vertex_positions[:,1], overall_Us1[-1], shade=False, cmap=cm.coolwarm, linewidth=0)
    #plot = plt.imshow(overall_Us[-1].reshape(grid.nx, grid.ny), cmap='plasma')
    np.save("output/AHL_ts.npy", overall_Us)
    plt.show()
...
if plot_u1:
    # plot the anmations
    pdf = matplotlib.backends.backend_pdf.PdfPages("output/AHL1.pdf")
    fig = plt.figure()
    ims = []
...
if plot_u2:
    pdf = matplotlib.backends.backend_pdf.PdfPages("output/AHL2.pdf")
    fig = plt.figure()
    ims = []
    for i in range(0,len(overall_Us1), frame_skip):
        print(i)
        #ax = fig.gca(projection = '3d')
        #plot = ax.bar3d(x, y, bottom, width, depth, Us[i], shade=True, color = 'b')
        fig.suptitle('time: ' + str(delta_t*i))
        #plot = ax.plot_surface(X, Y, Us[i].reshape(grid.nx, grid.ny), rstride=1, cstride=1, cmap=cm.coolwarm,
        #    linewidth=0, antialiased=False)
        plot = plt.imshow(overall_Us1[i].reshape(grid.nx, grid.ny), cmap='plasma', vmin = 0, vmax = 0.25)
