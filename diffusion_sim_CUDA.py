...
#should probably sub class this from NodeSim
class SynBioBrainCUDA(object):
    ...
    def __init__(self, grid_corners, nx, ny, checkerboard = False, dtype='float64'):
        ...
        self.build_synbio_brain_kernels()
    ...
    def build_synbio_brain_kernels(self):
        dtype = self.c_dtype
...
class NodeSimCUDA(object):
    ...
    def build_synbio_brain_kernels(self, dtype):
        ...
        self.get_Au_kernel = SourceModule("""
           __global__ void get_y(
            const int N, const {0} *xb,  {0} *yb, const int * non_boundary_nodesb, const int nx, const {0} dx, const {0} D)
            {{
                // THIS ONLY RUNS FOR NODES THAT ARENT BOUNDARY NODES
                int i = blockIdx.x*blockDim.x + threadIdx.x; // non boundary node number
                if (i <= N) {{ // check we havent run over
        ...
        self.add_next_u_kernel = SourceModule("""__global__ void add_next_u(
        const int number_of_nodes, {0} *Aub, const int *boundary_flagsb, const {0} dt,  const int t,  {0} *all_ub, {0} *current_ub, int * activated_nodesb, {0} * times_onb, {0} * times_offb, {0} production_rate)
        {{
        ...
        self.add_activated_kernel = SourceModule("""__global__ void add_activated(
        const int number_of_nodes, {0} *current_ub, int *activatedb, int * all_activatedb, int * input_verticesb, int * output_verticesb, const int t, {0} * psb){{
    ...
    def convert_params(self):
        ...
        if self.dtype == 'float64':
            dtype = 'double'
            self.D = np.float64(self.D)
            self.dx = np.float64(self.dx)
            self.production_rate = np.float64(self.production_rate)
            self.dt = np.float64(self.dt)
        elif self.dtype == 'float32':
            dtype = 'float'
            self.D = np.float32(self.D)
            self.dx = np.float32(self.dx)
            self.production_rate = np.float32(self.production_rate)
            self.dt = np.float32(dt)


    def create_arrays(self, u0, grid_corners, node_dim, input_indeces, output_indeces):
        #dt, n_steps, n_loops, ps, D, production_rate, boundary_cond, node_radius = params
        # create context, queue and memory flags
        ...
        output_vertices, one_hot_out = self.assign_vertices(self.vertex_positions, output_node_positions, self.node_radius)
        #output_vertices = np.array(range(0, len(self.vertex_positions)))
        #one_hot_out = np.ones_like(one_hot_out)
        input_vertices, one_hot_in = self.assign_vertices(self.vertex_positions, input_node_positions, self.node_radius)
        ...
        # create arrays
        if self.dtype == 'float64':
            all_u = np.zeros((self.n_steps, self.n_vertices), dtype = np.float64)
            Au = np.zeros(self.n_vertices, dtype = np.float64)
            current_u = np.array(u0, dtype = np.float64)

            #vertex_production_rates = np.float64(vertex_production_rates, dtype = np.float64)
            ps = np.array(self.ps, dtype = np.float64)
            node_times_on = np.zeros(self.n_vertices, dtype = np.float64)
            node_times_off = np.zeros(self.n_vertices, dtype = np.float64)

        elif self.dtype == 'float32':
            all_u = np.zeros((self.n_steps, self.n_vertices), dtype = np.float32)
            print('size: ', all_u.nbytes)
            Au = np.zeros(self.n_vertices, dtype = np.float32)
            current_u = np.array(u0, dtype = np.float32)
            ps = np.array(self.ps, dtype = np.float32)
            #vertex_production_rates = np.float32(vertex_production_rates, dtype = np.float32)
            node_times_on = np.zeros(self.n_vertices, dtype = np.float32)
            node_times_off = np.zeros(self.n_vertices, dtype = np.float32)
        ...
        # set activated to be the input nodes initially
        activated = np.array(one_hot_in, dtype = np.int32)
        all_activated = np.zeros(( self.n_steps, self.n_vertices), dtype = np.int32)
        boundary_nodes = np.array(self.get_boundary_nodes(), dtype = np.int32)
        self.n_boundary_vertices = np.int32(len(boundary_nodes))
        ...
        return [all_u, current_u, mask, Au, ps, node_times_on, node_times_off, all_activated, activated, boundary_nodes, non_boundary_nodes, one_hot_in, one_hot_out]
    ...
    def create_programs(self):
        ...
        if self.boundary_cond == 'zero':
            ...
            boundary_prg = self.boundary_kernel_periodic.get_function("boundary")
        elif self.boundary_cond == 'insulating':
            boundary_prg = self.boundary_kernel_insulating.get_function("boundary")
        else:
            raise Exception('invalid boundary conditions')
    ...
    def create_buffers(self, arrays):
        ...
        all_u, current_u, mask, Au, ps, node_times_on, node_times_off, all_activated, activated, boundary_nodes, non_boundary_nodes, one_hot_in, one_hot_out, one_hot_boundary = arrays
        ...
        one_hot_boundaryb = cuda.mem_alloc(one_hot_boundary.nbytes)
        cuda.memcpy_htod(one_hot_boundaryb, one_hot_boundary)
        ...
        return [all_ub, current_ub, maskb, Aub, psb, node_times_onb, node_times_offb, all_activatedb, activatedb, boundary_nodesb, non_boundary_nodesb, one_hot_inb, one_hot_outb, one_hot_boundaryb]

    def timestep(self, programs, buffers, t):
        Au_prg, boundary_prg, add_next_u_prg, add_activated_prg = programs
        all_ub, current_ub, Aub, psb, node_times_onb, node_times_offb, all_activatedb, activatedb, boundary_nodesb, non_boundary_nodesb, one_hot_inb, one_hot_outb, one_hot_boundaryb = buffers
        #dt, n_timesteps, n_loops, ps, D, production_rate, boundary_cond, node_radius = params
        # get Au buffer for the time step
        Au_prg(np.int32(self.n_vertices - self.n_boundary_vertices), current_ub, Aub, non_boundary_nodesb,one_hot_boundaryb, np.int32(self.nx), self.dx,  self.D, block = self.blockDim, grid=(int((self.n_vertices - self.n_boundary_vertices)//self.blockDim[0] + 1),int(1),int(1)))

        boundary_prg(np.int32(self.n_boundary_vertices), Aub, boundary_nodesb, current_ub, np.int32(self.n_vertices), np.int32(self.nx), self.dx, self.D,block = self.blockDim, grid=(int(self.n_boundary_vertices// self.blockDim[0] + 1),1,1))

        add_next_u_prg(np.int32(self.n_vertices), Aub, one_hot_boundaryb, dt, np.int32(t), all_ub, current_ub, activatedb, node_times_onb, node_times_offb,  self.production_rate, block = self.blockDim, grid=(self.n_vertices//self.blockDim[0] + 1,1,1))
        #add_next_u_prg.add_next_u(queue, (self.n_nodes,), None, Aub, boundary_flagsb, dt, np.int32(self.n_nodes), np.int32(t), activatedb, production_rate)#run kernel
        add_activated_prg( np.int32(self.n_vertices),current_ub, activatedb, all_activatedb, one_hot_inb, one_hot_outb, np.int32(t), psb, block = self.blockDim, grid=(int(self.n_vertices// self.blockDim[0] +1),1,1))
        #cl.enqueue_copy(queue, current_u, current_ub)
        #cl.enqueue_copy(queue, all_u, all_ub) # copy result from buffer
        #print('sum: ', sum(current_u))

    def run_sim(self, arrays, buffers, programs):

        all_u, current_u, mask, Au, ps, node_times_on, node_times_off, all_activated, activated, boundary_nodes, non_boundary_nodes, boundary_flags, one_hot_in, one_hot_out = arrays
        all_ub, current_ub, maskb, Aub, psb, node_times_onb, node_times_offb, all_activatedb, activatedb, boundary_nodesb, non_boundary_nodesb, boundary_flagsb, one_hot_inb, one_hot_outb = buffers
        ...
        for i in range(n_loops): # each loop allows copying from GPU to RAM
            ...
            for t in range(n_timesteps): # each loop runs in GPU memory
                if t %1000 == 0:
                    print(t)
                    print('--------------------------------------------------------------------')
                self.timestep(programs, buffers, t)
    ...
    def synbio_brain(self, u0, grid_corners, node_positions, input_indeces, output_indeces, params):

        params = self.convert_params(params)
        sefl.dt, self.n_timesteps, self.n_loops, self.ps, self.D, self.n_AHLs, self.production_rate, self.boundary_cond, self.node_radius = params

        arrays = self.create_arrays(u0, grid_corners, node_positions, input_indeces, output_indeces)# REFACTOR THIS INTO CLASS

        buffers = self.create_buffers(arrays)

        programs = self.create_programs()

        overall_Us, overall_activated = self.run_sim(arrays, buffers, programs)
        return overall_Us, overall_activated
