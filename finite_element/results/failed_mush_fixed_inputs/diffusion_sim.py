...
class SynBioBrainFE(object):

    """A Grid class that can handle two dimensional flat and surface grids."""

    def __init__(self, vertices, elements, grid_corners, dtype='float64'):
        """
        Initialize a grid.
        ...
        self.nIters = 0
        self.callIters = 0
        print(vertices[self.get_boundary_nodes()])
        print(self.get_boundary_nodes())
    ...
    @classmethod
    def from_file(cls, file_name, grid_corners,dtype ='float64'):
        ...
        return cls(points, elements, grid_corners, dtype)
    ...
    def get_boundary_nodes(self):
        ...
        boundaries = np.array(self.grid_corners, dtype = np.float64)
        boundary_nodes = []

        for index, node in enumerate(self.vertices):

            if np.any(np.abs(boundaries.T - node[0:2]) < 0.0000001): #will get all nodes on the square
    ...
    def synbio_brain(self, u0, dt, n_steps, input_vertices, output_vertices, ps, D, production_rate, is_automata = False):
        ...
        one_hot_all_node = np.append(input_vertices, output_vertices) # to allow different diff coeff inside nodes
        all_node_vertices = np.where(one_hot_all_node == 1)[0]


        print('len: ', len(all_node_vertices))
        print('sum: ', sum(one_hot_all_node))
        print(one_hot_all_node[all_node_vertices])
        #activated = np.zeros((nodes,1), dtype = np.int32)
        elements_inside_nodes = np.zeros(n, dtype = np.int32)
        for i, element in enumerate(element_mappings):


            if np.all([v in all_node_vertices for v in element]):
                elements_inside_nodes[i] = 1
        ...
        get_Au_kernel = """
           __kernel void get_y(
            __global const {0} *global_xb,__global const {0} *inv_J_Tsb,__global const {0} *integration_elementsb,__global const int *element_mappingb, __global {0} *global_yb, __global int *elements_inside_nodesb, const {0} D)
            {{
        ...
        {{
            ...
            */
            ...
            {0} production = activated_nodesb[i] * vertex_integration_elementsb[i] * exp_level * production_rate;
            {0} diffusion = -Aub[i];
            {0} degradation;
            {0} boundary;
            ...
            if (current_ub[i] > 0.0005) {{
                degradation = -0.0005 * vertex_integration_elementsb[i];
            }} else {{
                degradation = -current_ub[i] * vertex_integration_elementsb[i];
            }}
            ...
            */
            ...
            {0} du =  1/Mb[i]*(production + diffusion + boundary);
            ...
            if (boundary_flagsb[i] == 1){{
                /*
                if (current_ub[i] < 1) {{
                    boundary = 0.05 * 0.000003 * current_ub[i];
                }} else {{
                    boundary = 0.05 * 0.000003;
                }}
                */
                du = 0;
            }}
        ...
        elements_inside_nodesb = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=elements_inside_nodes)
        # create buffers
        ...
        print('shape boissss: ', all_u.shape, all_activated.shape)
        for t in range(n_steps):
            if t%100 == 0:
                print(t)
            # get Au buffer for the time step
            Au_prg.get_y(queue, (n,),None, current_ub, inv_J_Tsb, integration_elementsb, element_mappingb, unsummed_Aub, elements_inside_nodesb, D)
...
class SynBioBrainFD(object):
    ...
    def get_node_position(self,node_number):
    ...
    def get_boundary_nodes(self):
        ...
        for i in range(self.n_nodes):
            ...
            if np.any(np.abs(boundaries.T - np.array(position)) < 0.0000001): #will get all nodes on the square
    ...
    def synbio_brain(self, u0, dt, n_steps, input_vertices, output_vertices, ps, D, production_rate, is_automata = False):
        ...
        if self.dtype == 'float64':
            ...
            all_u = np.zeros((n_steps+1, self.n_nodes), dtype = np.float64)

            dt = np.float64(dt)
            unsummed_Au = np.zeros((self.n_nodes,3), dtype = np.float64) # output array
            summed_Au = np.zeros(self.n_nodes, dtype = np.float64)
            current_u = np.array(u0, dtype = np.float64)
            ...
            vertex_times_on = np.zeros(self.n_nodes, dtype = np.float64)
            vertex_times_off = np.zeros(self.n_nodes, dtype = np.float64)
            ...
            all_u = np.zeros(( n_steps + 1, self.n_nodes), dtype = np.float32)
            dt = np.float32(dt)
            unsummed_Au = np.zeros((self.n_nodes,3), dtype = np.float32) # output array

            summed_Au = np.zeros(self.n_nodes, dtype = np.float32)
            current_u = np.array(u0, dtype = np.float32)
            ...
            vertex_times_on = np.zeros(self.n_nodes, dtype = np.float32)
            vertex_times_off = np.zeros(self.n_nodes, dtype = np.float32)
        ...
        self.nx = np.int32(self.nx)
        #activated = np.zeros((nodes,1), dtype = np.int32)

        # set activated to be the input nodes initially
        activated = input_vertices
        all_activated = np.zeros(( n_steps + 1, self.n_nodes), dtype = np.int32)
        boundary_nodes = np.array(self.get_boundary_nodes(), dtype = np.int32)
        ...
        get_Au_kernel = """
           __kernel void get_y(
            __global const {0} *xb, __global {0} *yb, const int nx,  const {0} dx, const {0} D)
            {{

                int n = get_global_id(0); // node number
        ...
        */
        ...
        for t in range(n_steps):
            if t%100 == 0:
                print(t)
            # get Au buffer for the time step
            Au_prg.get_y(queue, (self.n_nodes,),None, current_ub, unsummed_Aub, self.nx, self.dx, D)

            # sum the Au
            #boundary_prg.boundary(queue, (len(boundary_nodes),), None, summed_Aub, boundary_nodesb)
            # calculate next u and add to the buffer of all us

            add_next_u_prg.add_next_u(queue, (self.n_nodes,), None, summed_Aub, boundary_flagsb, dt, np.int32(self.n_nodes), np.int32(t), all_ub, current_ub, activatedb, vertex_times_onb, vertex_times_offb,  production_rate)#run kernel
            add_activated_prg.add_activated(queue, (self.n_nodes,), None, current_ub, activatedb, all_activatedb, input_verticesb, output_verticesb, np.int32(self.n_nodes), np.int32(t), psb)
...
class SynBioBrainFD_old(object):
    ...
    def synbio_brain(self, u0, dt, n_steps, input_nodes, output_nodes, ps, D, production_rate, is_automata = False):
        # M doesnt depend on U so can calculate it once outside the loop
        ...
        get_Au_kernel = """
           __kernel void get_y(
            __global const {0} *xb, __global {0} *yb, __global const int * non_boundary_nodesb, const int nx, const {0} dx, const {0} D)
            {{
        ...
        add_activated_prg = cl.Program(ctx, add_activated_kernel).build()
        print(Au.shape)
        print(unsummed_Au.shape)

        for t in range(n_steps):


            # get Au buffer for the time step
            Au_prg.get_y(queue, (self.n_nodes - n_boundary_nodes,),None,current_ub, Aub, non_boundary_nodesb, np.int32(self.nx), self.dx,  D)

            #boundary_prg.boundary(queue, (len(boundary_nodes),), None, Aub, boundary_nodesb)
            # calculate next u and add to the buffer of all us
            add_next_u_prg.add_next_u(queue, (self.n_nodes,), None, Aub, boundary_flagsb, dt, np.int32(self.n_nodes), np.int32(t), all_ub, current_ub, activatedb, node_times_onb, node_times_offb, production_rate)
        ...
        all_u = all_u.reshape(n_steps+1,self.n_nodes)
        cl.enqueue_copy(queue, all_activated, all_activatedb)
        all_activated = all_activated.reshape(n_steps+1,self.n_nodes)
